...
pub struct Pasta {
    ...
    pub custom_path: Option<String>, // 新增：自定义路径字段
    pub random_string_path: Option<String>, // 随机字符串路径
}
...
impl Pasta {
    ...
    /// 获取pasta的访问路径，优先级：自定义路径 > 随机字符串路径 > 动物名称/hash
    pub fn get_path(&self) -> String {
        if let Some(custom_path) = &self.custom_path {
            custom_path.clone()
        } else if let Some(random_string_path) = &self.random_string_path {
