    ...
    let pasta_iter = stmt
        .query_map([], |row| {
            Ok(Pasta {
                ...
                custom_path: row.get(17)?,
                random_string_path: row.get(18).unwrap_or(None), // 向后兼容：如果字段不存在则为None
            })
        })
...
pub fn insert(pasta: &Pasta) {
    ...
    conn.execute(
        ...
        CREATE TABLE IF NOT EXISTS pasta (
            ...
            pasta_type TEXT NOT NULL,
            custom_path TEXT,
            random_string_path TEXT
        );",
        params![],
    )
    .expect("Failed to create SQLite table for Pasta!");

    // 添加新字段的迁移（如果表已存在但缺少新字段）
    let _ = conn.execute(
        "ALTER TABLE pasta ADD COLUMN random_string_path TEXT;",
        params![],
    ); // 忽略错误，因为字段可能已经存在
    ...
    conn.execute(
        ...
        params![
            ...
            pasta.pasta_type,
            pasta.custom_path.as_deref(),
            pasta.random_string_path.as_deref(),
        ],
    )
    .expect("Failed to insert pasta.");
}
